...
local function cINST()
    ...
    -- 同期ズレとアイテム使用不可の不具合の原因となるためdoDesyncの呼び出しを削除
    -- doDesync()
...
local function createTopInfo()
    local oldGui = coreGui:FindFirstChild("PXHUB_TOP_INFO")
    if oldGui then oldGui:Destroy() end

    local statsGui = Instance.new("ScreenGui")
    statsGui.Name = "PXHUB_TOP_INFO"
    ...
    title.BackgroundTransparency = 1
    title.Text = "PXHUB V1.9"
    title.TextColor3 = Color3.new(1, 1, 1)
    title.Font = Enum.Font.GothamBold
    title.TextSize = 20

    local titleColors = ColorSequence.new{
    ...
    run.RenderStepped:Connect(function(dt)
        fps += 1
        if gradStroke.Parent then gradStroke.Rotation = (gradStroke.Rotation + 120 * dt) % 360 end
        if gradTitle.Parent then gradTitle.Rotation = (gradTitle.Rotation - 160 * dt) % 360 end
        if gradDiscord.Parent then gradDiscord.Rotation = (gradDiscord.Rotation - 160 * dt) % 360 end
        if tick() - last >= 1 then
            local ping = math.floor(statsService.Network.ServerStatsItem["Data Ping"]:GetValue())
            stats.Text = "FPS: <font transparency='0.5'>"..fps.."</font> | PING: <font transparency='0.5'>"..ping.."</font>"
            fps = 0
            last = tick()
        end
    end)
...
coreGui.ChildRemoved:Connect(function(child)
    if child.Name == "PXHUB_TOP_INFO" then
        task.wait(0.1)
        createTopInfo()
    end
end)
